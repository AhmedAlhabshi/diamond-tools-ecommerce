...
export default function DesktopHeader({ locale }: Props) {
...
/>
          ...
          {/* SEARCH BAR - Centered vertically with logo */}
            ...
            <div className="relative group">
              <input
                ...
                className={`w-full bg-gray-100 border-transparent border focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 rounded-full py-2.5 transition-all outline-none text-sm ${
                  locale === "ar" ? "pr-12 pl-5 text-right" : "pl-12 pr-5"
                }`}
              />
              <Search
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  {results.length > 0 ? (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-gray-50 rounded-lg flex items-center justify-center p-1">
                            <img src={item.images?.[0] || "/placeholder.png"} className="w-full h-full object-contain" alt="" />
                          </div>
                    ...
                    <div className="p-8 text-center">
                      <p className="text-gray-400 text-sm">{search ? t("noResults") : "Start typing to search..."}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* CART - Centered vertically with logo and search */}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden relative">
            <button
              ...
              className={`absolute top-5 z-10 p-2 hover:bg-gray-100 rounded-full transition-colors ${locale === "ar" ? "left-5" : "right-5"}`}
            >
            ...
            <div className="p-8">
              {submitted ? (
                <div className="text-center py-10 space-y-6">
                  ...
                  <button onClick={() => setShowQuote(false)} className="w-full bg-gray-900 text-white py-3 rounded-xl font-bold">{t("close")}</button>
                </div>
              ) : (
                <>
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">{t("quoteTitle")}</h2>
                  <form
                    onSubmit={async (e) => {
                      e.preventDefault();
                      setLoading(true);
                      ...
                      setLoading(false);
                      if (res.ok) setSubmitted(true); else toast.error(t("error"));
                    }}
                    className="space-y-4"
                  >
                    <input name="name" placeholder={t("fullName")} required className="w-full border border-gray-200 p-3 rounded-xl text-sm outline-none focus:border-blue-500" />
                    <input name="phone" placeholder={t("phone")} required className="w-full border border-gray-200 p-3 rounded-xl text-sm outline-none focus:border-blue-500" />
                    <textarea name="message" placeholder={t("message")} rows={3} className="w-full border border-gray-200 p-3 rounded-xl text-sm outline-none focus:border-blue-500 resize-none" />
                    <input type="file" accept="application/pdf" onChange={(e) => setQuoteFile(e.target.files?.[0] || null)} className="w-full text-xs" />
